package com.simiyu.enock.enock.service.impl;
...
import com.simiyu.enock.enock.config.FileStorageProperties;
import com.simiyu.enock.enock.exception.FileStorageException;
import com.simiyu.enock.enock.exception.MyFileNotFoundException;
...
public class FileStorageServiceImpl implements FileStorageService{
	...
	@Autowired
	public FileStorageServiceImpl(FileStorageProperties fileStoreProperties) {
		this.fileStorageLocation = Paths.get(fileStoreProperties.getUploadDir()).toAbsolutePath().normalize();		
		try {
			...
			throw new FileStorageException(AppConstants.INVALID_FILE_FORMAT);
				File f = new File(AppConstants.TEMP_DIR+file.getOriginalFilename());
					 ...
					 int width = image.getWidth();
					 	...
					 	try {
					 		if(fileName.contains(AppConstants.INVALID_FILE_DELIMITERS)) {
